...
if ($saveHistory) {
    ...
    ")->execute([
        ...
        $level, $language, 'internship', $provider . '/' . $activeModel
    ]);
...
$prompt = <<<PROMPT
You are an expert internship program designer for tech companies.

Create days {$startDay} to {$endDay} of a {$totalDays}-day "{$topic}" internship program for {$level} level interns in {$language} language.

This is a PRACTICAL INTERNSHIP — not a course. Each day must have:
...
- Tasks must build progressively — Day 1 onboarding → Day {$totalDays} final presentation
- Each day covers DIFFERENT and UNIQUE tasks — NO repetition
- {$quizNote}
- image_query must be specific and descriptive

Return ONLY valid JSON array with exactly {$bDays} objects. No markdown, no backticks, no explanation — pure JSON only:
[{"day":{$startDay},"title":"Short task title","topics":["task 1","task 2","task 3"],"image_query":"specific query","has_quiz":false}]
PROMPT;

...
if ($currentLesson) {
    foreach ($allLessonsFlat as $idx => $l) {
        if ($l['id'] == (int)$currentLesson['id']) {
            ...
            if ($idx < count($allLessonsFlat) - 1 && empty($allLessonsFlat[$idx + 1]['is_locked'])) {
                $nextLesson = $allLessonsFlat[$idx + 1];
            }
            break;
        }
    }
    ...
    <div class="flex-1 overflow-y-auto bg-white mobile-content">
        <?php if ($currentLesson): ?>
            ...
            <div class="max-w-4xl mx-auto p-4 sm:p-6 md:p-8">
                ...
                <?php if ($currentLessonLocked): ?>
                    <div class="bg-gradient-to-br from-gray-50 to-gray-100 border-2 border-gray-200 rounded-2xl p-8 sm:p-14 text-center">
                        ...
                        <p class="text-sm text-gray-500 max-w-md mx-auto mb-6">
                            <?php if ((int)$currentLesson['unlock_in'] === 1): ?>
                                Come back tomorrow — the next day unlocks in 1 day.
                            <?php else: ?>
                                New content is released one day at a time. This day unlocks in <?php echo (int)$currentLesson['unlock_in']; ?> days.
                            <?php endif; ?>
                        </p>
                ...
                <?php if (!$currentLessonLocked && $lessonHasQuiz && $isWeeklyQuizLesson): ?>
